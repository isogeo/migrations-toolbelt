...
"""
    Name:         Migration script for "Ville de Niort" workgroup metadata in 2020
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/niort/_logs/migration_niort.log"), "a", 5000000, 1
    )
    ...
    # prepare csv reading
    input_csv = Path(r"./scripts/niort/csv/correspondances.csv")
    fieldnames = [
        "source_uuid",
        "source_title",
        "source_name",
    ...
    with input_csv.open() as csvfile:
        ...
        for row in reader:
            ...
            if reader.line_num > 1:
                ...
                if trg_uuid == "NULL" or src_name == trg_name:
                    continue
    ...
    if int(environ.get("BACKUP")) and len(li_to_backup):
        ...
        backup_path = Path(r"./scripts/niort/_output/_backup")
    ...
    csv_result = Path("./scripts/niort/csv/migrated_{}.csv".format(datetime.now().timestamp()))
    ...
    if len(li_failed) > 0:
        ...
        csv_failed = Path(r"./scripts/niort/csv/migrate_failed.csv")
