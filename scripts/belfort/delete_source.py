...
"""
    Name:         Script to delete "Grand Belfort" source metadatas after migration
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/belfort/_logs/delete_source.log"), "a", 5000000, 1
    )
    ...
    # load the list of metadatas to delete from migration csv report file
    li_dlt_uuid = []

    # prepare csv file reading
    input_csv = Path(r"./scripts/belfort/csv/bilan_migration_Belfort.csv")
    fieldnames = [
        "source_uuid",
        "source_title",
        "source_name",
    ...
    if environ.get("BACKUP") == "1" and len(li_dlt_uuid) > 0:
        ...
        backup_path = Path(r"./scripts/belfort/_output/_source_deletion_backup")
    ...
    for uuid in li_dlt_uuid:
        ...
        md_cat_uuid = [cat.get("_id") for cat in isogeo.catalog.metadata(uuid)]
        if source_cat_uuid in md_cat_uuid and migrated_cat_uuid not in source_cat_uuid:
            pass
        else:
            if source_cat_uuid not in md_cat_uuid:
                logger.info("{} metadata won't be deleted because it is not associated to {} Source catalog".format(uuid, source_cat_uuid))
            else:
                logger.info("{} metadata won't be deleted because it is associated to {} Migrated catalog".format(uuid, migrated_cat_uuid))
            continue
        try:
