...
"""
    Name:         Migration script for "Conseil Départemental du Val de Marne (CD 94)" workgroup metadata in 2020
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/cd94/_logs/migration_cd94.log"), "a", 5000000, 1
    )
    ...
    li_to_backup = []
    # prepare csv reading
    input_csv = Path(r"./scripts/cd94/csv/correspondances.csv")
    fieldnames = [
        "source_uuid",
        "source_title",
        "source_name",
    ...
    if environ.get("BACKUP") == "1" and len(li_to_backup):
        ...
        backup_path = Path(r"./scripts/cd94/_output/_backup")
    ...
    csv_result = Path("./scripts/cd94/csv/migrated_{}.csv".format(datetime.now().timestamp()))
    ...
    if len(li_failed) > 0:
        ...
        csv_failed = Path(r"./scripts/cd94/csv/migrate_failed.csv")
