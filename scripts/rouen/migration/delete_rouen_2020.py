...
"""
    Name:         Script to delete Rouen source metadatas migrated in 2020
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/rouen/migration/_logs/delete_rouen.log"), "a", 5000000, 1
    )
    ...
    if environ.get("BACKUP") == "1" and len(li_md_to_delete):
        ...
        backup_path = Path(r"./scripts/rouen/migration/_output/_backup_deleted")
    ...
    for md_uuid in li_md_to_delete:
        # Refresh API token if needed
        if default_timer() - auth_timer >= 250:
            logger.info("Manually refreshing token")
            backup_mng.isogeo.connect(
        ...
        isogeo.metadata.delete(metadata_id=md_uuid)
