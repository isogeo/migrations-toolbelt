...
"""
    Name:         Migration script for "Caen la mer" workgroup metadata in 2020
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/caen_la_mer/_logs/migration_caen.log"), "a", 5000000, 1
    )
    ...
    li_to_backup = []
    # prepare csv reading
    input_csv = Path(r"./scripts/caen_la_mer/csv/correspondances.csv")
    fieldnames = [
        "source_name",
        "source_uuid",
        "target_uuid",
            ...
            if src_uuid != "source_uuid":
                ...
                # time to test UUID and nam of target metadata
                    ...
                    # added to a list
                        ...
                        to_migrate = (src_uuid, src_name)
                        li_src_to_migrate.append(to_migrate)

                        li_trg_to_migrate.append(trg_uuid)
    ...
    if environ.get("BACKUP") == "1" and len(li_to_backup):
        ...
        backup_path = Path(r"./scripts/caen_la_mer/_output/_backup")
    ...
    csv_result = Path("./scripts/caen_la_mer/csv/migrated_{}.csv".format(datetime.now().timestamp()))
    ...
    if len(li_failed) > 0:
        logger.info("{} metadatas haven't been migrated. Launch the script again pointing to 'migrate_failed.csv' file\n".format(len(li_failed)))
        csv_failed = Path(r"./scripts/caen_la_mer/csv/migrate_failed.csv")
