...
"""
    Name:         Script to clean attribute scale, precision, length or type related corrupted events added by Isogeo Scan
    Author:       Isogeo
...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/misc/events/_logs/attributeInfos_events_cleaner.log"), "a", 5000000, 1
    )
    ...
    for tup in li_events_to_clean:
        ...
        if " attribute " in event.description:
            part_count = 0
            new_description = ""
    ...
    csv_path = Path("./scripts/misc/events/csv/attributeInfos_cleaner_{}.csv".format(str(datetime.now().timestamp()).split(".")[0]))
