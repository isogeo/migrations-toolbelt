...
if __name__ == "__main__":
    ...
    log_file_handler = RotatingFileHandler(
        Path("./scripts/GRDF/migration_jura.log"), "a", 5000000, 1
    )
    ...
    li_to_backup = []
    # to store source metadata uuid and name that passe the tests
    li_src_to_migrate = []
    # store all source uuid that appear in the mapping table
    src_found = []
    # to store target metadata uuid and name that passe the tests
    li_trg_to_migrate = []
    # store all target uuid that appear in the mapping table
    trg_found = []
    # prepare csv reading
    input_csv = Path(r"./scripts/GRDF/migrate_failed.csv")
    fieldnames = [
        "source_uuid",
        "source_name",
        "target_name",
    ...
    with input_csv.open() as csvfile:
        ...
        for row in reader:
            ...
            if src_uuid != "source_uuid":
                ...
                # if UUID and name of source metadata have passed all checks,
    ...
    # # ------------------------------------- BACKUP -------------------------------------
    # backup_mngr = BackupManager(api_client=isogeo, output_folder="./scripts/GRDF/output")
        ...
        # dupplicate the metadata
            ...
            try:
                md_dst = src_migrator.import_into_other_metadata(
                    ...
                    copymark_title=True,  # FALSE EN PRDO
                    destination_metadata_uuid=trg_uuid,
                    associate_catalogs=False,
    ...
    csv_result = Path(r"./scripts/GRDF/migrated_bis.csv")
    ...
    if len(li_failed) > 0:
        ...
        csv_failed = Path(r"./scripts/GRDF/migrate_failed.csv")
